...
int main(){
    ...
    printf("initial host matrix: %f s\n", iElaps);
    ...
    printf("add matrix at host side: %f s\n", iElaps);
    ...
    iElaps = cpuSecond() - iStart;
    printf("add matrix at device cost %f s, sumMatrixOnGPU2D <<<(%d,%d), (%d,%d)>>>\n", iElaps, grid.x, grid.y, block.x, block.y);
    ...
    cudaDeviceSynchronize();
    iElaps = cpuSecond() - iStart;
    printf("add matrix at device cost %f s, sumMatrixOnGPU1D <<<(%d,%d), (%d,%d)>>>\n", iElaps, grid1D.x, grid1D.y, block1D.x, block1D.y);
